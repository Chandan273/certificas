...
<AdminLayout>
    ...
    <div class="pa-8 pa-sm-4 pa-md-4 pa-lg-6 widget-card">
        <DxDataGrid :data-source="dataSource" :show-borders="true" key-expr="id">
            ...
            <DxEditing :allow-updating="true" :allow-adding="true" :allow-deleting="true" mode="popup">
                ...
                <DxForm>
                    <DxItem :col-count="2" :col-span="2" item-type="group">
                        <DxItem data-field="number" :validation-rules="[{ type: 'required', message: 'Number is required' }]" />
                        <DxItem data-field="name" :validation-rules="[{ type: 'required', message: 'Name is required' }]" />
                        <DxItem data-field="contact" :validation-rules="[{ type: 'required', message: 'Contact is required' }]" />
                        <DxItem data-field="organisation_number" :validation-rules="[{ type: 'required', message: 'organisation Number is required' }]" />
                        <DxItem data-field="email" :validation-rules="[{ type: 'required', message: 'Email Address is Required' }, { type: 'email' }]" />
                        <DxItem data-field="www" caption="Website" :validation-rules="[{ type: 'required', message: 'Website is required' }, { type: 'pattern', pattern: /^(https?:\/\/)?([\w\d]+\.)+[\w\d]+$/, message: 'Invalid website address' }]" />

                        <DxItem :col-count="2" :col-span="2" item-type="group" caption="Customer Address">
                            <DxItem :col-span="2" :editor-options="{ height: 100 }" data-field="address" editor-type="dxTextArea" :validation-rules="[{ type: 'required', message: 'Address is required' }]" />
                        </DxItem>
                        <DxItem data-field="phone" caption="Phone Number" :validation-rules="[{ type: 'required', message: 'Phone number is required' }, { type: 'pattern', pattern: /^[\d()\-+ ]{10,15}$/, message: 'Invalid phone number, phone number should have minimum length 10 and maximum length 15' }]" />
                        <DxItem data-field="zip" :validation-rules="[{ type: 'required', message: 'Zip is required' }, { type: 'pattern', pattern: /^[\d()\-+ ]+$/, message: 'Invalid Zip number'}]" />
                        <DxItem data-field="city" :validation-rules="[{ type: 'required', message: 'City is required' }]" />
                        <DxItem data-field="country" :validation-rules="[{ type: 'required', message: 'Country is required' }]">
            ...
            <DxColumn data-field="organisation_number" caption="Organisation Number" />
