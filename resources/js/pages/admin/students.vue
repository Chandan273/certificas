...
<AdminLayout>
    ...
    <div class="pa-8 pa-sm-4 pa-md-4 pa-lg-6 widget-card">
                    ...
                    <v-dialog v-model="dialog" width="500">
                        ...
                        <v-card>
                            <div class="d-flex align-center justify-space-between">
                                ...
                                <v-icon class="mx-4" color="#333" @click="dialog = false">
                                    mdi-close
                                </v-icon>
                            </div>
                            <v-divider></v-divider>
                            ...
                            <v-divider></v-divider>
                            <v-card-actions>
                                <p @click="downloadCSV" class="secondary-dark">
        ...
        <DxDataGrid :data-source="dataSource" :remote-operations="true" :show-borders="true" key-expr="id" @exporting="onExporting">
          ...
          />
            ...
            <DxEditing :allow-updating="true" :allow-adding="true" :allow-deleting="true" mode="popup">
                <DxPopup :show-title="true" :width="700" :height="375" title="Student Info" />
                <DxForm>
                    <DxItem :col-count="2" :col-span="2" item-type="group">
                        <DxItem data-field="course_id" :validation-rules="[
                  { type: 'required', message: 'Course is required' },
                ]" />
                        <DxItem data-field="name" :validation-rules="[
                  { type: 'required', message: 'Name is required' },
                ]" />
                        <DxItem data-field="email" :validation-rules="[
                  { type: 'required', message: 'Email Address is Required' },
                  { type: 'email' },
                ]" />
                        <DxItem data-field="birth_date" :validation-rules="[
                  { type: 'required', message: 'Date of Birth is Required' },
                  {
                    type: 'custom',
                    validationCallback: validateAge,
                    message: 'Must be 15 years old or older',
                  },
                ]" />
                        <DxItem data-field="birth_place" :validation-rules="[
                  { type: 'required', message: 'Birth Place is required' },
                ]" />
                    </DxItem>
                </DxForm>
            </DxEditing>
            ...
            <template #importAction>
                <DxButton icon="import" @click="importCSV" title="Import CSV file" />
            </template>
        </DxDataGrid>
    </div>
</AdminLayout>
