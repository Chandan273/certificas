<template>
  <v-app-bar title="Certificas" class="Certificas">
    <v-app-bar-nav-icon slot="start" variant="text" @click.stop="drawer = !drawer"
      icon="mdi-format-align-left"></v-app-bar-nav-icon>
  </v-app-bar>

  <v-navigation-drawer v-model="drawer" :mini-variant="miniVariant" permanent>
    <v-list density="compact" nav>
      <template v-for="(items, i) in adminItems" :key="i">
        <v-list-item :prepend-icon="items.icon" :title="items.title" :class="{ active: isActive }" :value="items.title"
          :to="items.to"></v-list-item>
      </template>
    </v-list>

    <v-list-item link prepend-icon="mdi mdi-logout" title="Logout" @click="logout()"></v-list-item>
  </v-navigation-drawer>
</template>
  ...
  data() {
    return {
      ...
      adminItems: [
        ...
        { title: 'Students', icon: 'mdi-book-open-variant', to: '/students' },
